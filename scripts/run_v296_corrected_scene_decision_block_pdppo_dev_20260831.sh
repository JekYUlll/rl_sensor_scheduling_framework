...
# V296 isolates semi-Markov decision-block credit on the corrected scene.
# It matches V294 and changes only PPO credit assignment: forced minimum-dwell
# rows contribute to the decision-block return but are not separate policy
# transitions.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
if [[ "$#" -gt 0 ]]; then SEEDS=("$@"); else SEEDS=(6821 6822); fi

run_one() {
  local seed="$1" gpu="$2"
  (
    export CUDA_VISIBLE_DEVICES="$gpu"
    export RUN_PREFIX="v296_corrected_scene_decision_block_pdppo_dev"
    export LOG_PREFIX="v296_corrected_scene_decision_block_pdppo"
    export TOTAL_TIMESTEPS=50000
    export REWARD_PROXY_MODE=forecast EVENT_START_PROB=1.0
    export DECISION_ONLY_POLICY_UPDATES=0 DECISION_BLOCK_CREDIT=1
  ...
  ) >"logs/v296_corrected_scene_decision_block_pdppo_seed${seed}.log" 2>&1
}
