...
OUT_DIR="reports/v31_static_break_v25_v24_event_selective_laser_low_budget_profile_scan_dwell12_gate_seed45_h082_20260620"
mkdir -p "$OUT_DIR"

# Phase 18 / V25 diagnostic:
# keep the V24 event-selective-laser information structure, but squeeze the
# per-step budget below the calm-laser and met-laser compact shortcuts.
# B=1.03 is the minimum budget that still allows surface+shielded+FC4, B=1.05
# gives a small operational margin, and B=1.08 is the boundary where the
# calm-laser static combination returns.
