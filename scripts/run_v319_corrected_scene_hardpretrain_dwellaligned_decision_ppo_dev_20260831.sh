...
# V319 repeats the V318 hard forecast-value initialization plus decision-only
# PPO combination while aligning the hard teacher lookahead with the
# environment minimum dwell (six steps). No bandit-dependent signal or
# test-time event label is used.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-6991 6992})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v319_corrected_scene_hardpretrain_dwellaligned_decision_ppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v319_corrected_scene_hardpretrain_dwellaligned_decision_ppo}"
  ...
  ) >"logs/v319_corrected_scene_hardpretrain_dwellaligned_decision_ppo_seed${seed}.log" 2>&1
}
