...
# V323 keeps V322's corrected scene and hard teacher initialization, then
# trains a candidate-level forecast-value head on on-policy decision states.
# The head is supervised only by the frozen forecast oracle and is not tied to
# any handcrafted baseline or privileged final-test label.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7031 7032})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v323_onpolicy_forecast_value_transfer_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v323_onpolicy_forecast_value_transfer_pdppo}"
  ...
  ) >"logs/v323_onpolicy_forecast_value_transfer_pdppo_seed${seed}.log" 2>&1
}
