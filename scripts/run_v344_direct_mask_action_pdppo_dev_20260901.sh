...
# V344 tests a direct state--candidate-mask utility inside the masked PPO
# actor. It retains the V319/V341 forecast-loss reward, hard feasibility mask,
# scene, dwell protocol, and training-only forecast-value initialization. No
# comparator action, bandit signal, or final-test label is used.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6871 6872)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7361 7362})
...
run_one() {
  ...
  (
    ...
    export FORECAST_VALUE_AUX_COEF=0 FORECAST_VALUE_RANKING_COEF=0 CANDIDATE_INTERACTION_SCORE=0
    export DIRECT_MASK_ACTION_SCORE=1 FACTORIZED_ACTION_POLICY=0
  ...
  ) >"logs/v344_direct_mask_action_pdppo_seed${seed}.log" 2>&1
}
