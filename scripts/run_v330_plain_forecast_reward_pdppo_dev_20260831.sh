...
# V330 isolates reward-target alignment.  It retains V328's joint actor,
# context, teacher initialization, and feasibility protocol, but uses the
# unnormalised forecast-loss reward that matches the ordinary primary endpoint.
# No bandit-dependent signal or test-time event label is used.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7081 7082})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v330_plain_forecast_reward_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v330_plain_forecast_reward_pdppo}"
    export TOTAL_TIMESTEPS=50000 POLICY_SEED="$policy_seed" CONTROL_SOURCE_RUN_DIR="$source"
    export REWARD_PROXY_MODE=forecast REWARD_LOSS_NORMALIZATION=none EVENT_START_PROB=1.0
  ...
  ) >"logs/v330_plain_forecast_reward_pdppo_seed${seed}.log" 2>&1
}
