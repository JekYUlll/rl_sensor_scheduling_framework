...
export RUN_PREFIX="${RUN_PREFIX:-v31_metpair_backbone_context_ortholinear_strongteacher}"
export BUDGET_LABEL="${BUDGET_LABEL:-h075ctxolst}"
export DATE_TAG="${DATE_TAG:-20260620}"

# Same orthogonal-linear generator as the ortholinear branch; only the
# learned-policy curriculum is strengthened.
...
# Teacher/curriculum branch. The replay gate's best structural policy on
# seed41 used lead=10, so align oracle teacher, AWBC teacher, and subtype aux.
...
"${PY:-python}" scripts/72_v31_collect_metpair_strongclaim.py \
  --runs "${RUNS[@]}" \
  --out-dir "reports/aggregate/metpair_backbone_context_ortholinear_strongteacher_${#SEEDS[@]}seed_macro_${DATE_TAG}"

"${PY:-python}" scripts/72_v31_collect_metpair_strongclaim.py \
  --runs "${RUNS[@]}" \
  --router-eval-dir . \
  --out-dir "reports/aggregate/metpair_backbone_context_ortholinear_strongteacher_${#SEEDS[@]}seed_raw_macro_${DATE_TAG}"
