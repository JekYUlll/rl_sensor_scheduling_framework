...
# V325 adds a state-candidate interaction head to the masked PPO actor.  The
# head scores each feasible subset from the encoded online state and its
# candidate embedding; no comparator action, event label, or bandit objective
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v325_candidate_interaction_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v325_candidate_interaction_pdppo}"
  ...
  ) >"logs/v325_candidate_interaction_pdppo_seed${seed}.log" 2>&1
}
