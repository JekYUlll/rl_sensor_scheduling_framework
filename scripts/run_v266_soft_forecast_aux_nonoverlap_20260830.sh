...
# V266 adds only the existing frozen-forecaster soft forecast-value auxiliary
# target to V265's non-overlapping decision objective. It does not use bandit
# actions, event labels at execution, or duty quotas.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
PY="${PY:-$HOME/.conda/envs/darts/bin/python}"
if [[ "$#" -gt 0 ]]; then SEEDS=("$@"); else SEEDS=(5601 5602); fi

run_one() {
  local seed="$1" gpu="$2"
  (
    export CUDA_VISIBLE_DEVICES="$gpu"
    export RUN_PREFIX="v266_soft_forecast_aux_nonoverlap_pdppo_sixch_dev"
    ...
    export AWBC_COEF=0 AWBC_DECAY_TIMESTEPS=0 AWBC_EVENT_ONLY=0 AWBC_LABEL_STRIDE=4 BC_PRETRAIN_STEPS=0 BC_PRETRAIN_EPOCHS=0 BC_PRETRAIN_LOSS_COEF=0
    ...
    export FORECAST_VALUE_HEAD=0 FORECAST_VALUE_HEAD_MODE=mask_structured FORECAST_VALUE_HEAD_SCALE=0.0 FORECAST_VALUE_HEAD_HIDDEN_DIM=64 FORECAST_VALUE_HEAD_IGNORE_QUALITY=0
    export FORECAST_VALUE_AUX_COEF=0.20 FORECAST_VALUE_AUX_STRIDE=32 FORECAST_VALUE_AUX_LOOKAHEAD_STEPS=6 FORECAST_VALUE_AUX_LOSS=soft_ce FORECAST_VALUE_AUX_TEMPERATURE=1.0 FORECAST_VALUE_RANKING_COEF=0.10
  ...
  ) >"logs/v266_soft_forecast_aux_nonoverlap_pdppo_sixch_seed${seed}.log" 2>&1
}
