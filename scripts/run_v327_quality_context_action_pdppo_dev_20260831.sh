...
# V327 tests an explicit online quality/context-to-candidate utility head. It
# retains the V325 protocol and removes only the candidate-interaction head;
# the quality and alert features are observed online and no event labels or
# comparator actions are supplied at evaluation.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7045 7046})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v327_quality_context_action_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v327_quality_context_action_pdppo}"
  ...
  ) >"logs/v327_quality_context_action_pdppo_seed${seed}.log" 2>&1
}
