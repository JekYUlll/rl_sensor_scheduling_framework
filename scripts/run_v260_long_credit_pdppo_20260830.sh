...
# V260 tests a longer standard PPO credit horizon after V259's executable-mask
# correction. The forecast-loss reward, action geometry, and policy inputs are
# unchanged; only gamma and GAE lambda are increased.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v260_long_credit_pdppo_sixch_dev"
    ...
    export GAMMA=0.997 GAE_LAMBDA=0.98
    bash scripts/run_v32_flexible_subset_pilot_20260822.sh "$seed"
  ) >"logs/v260_long_credit_pdppo_sixch_seed${seed}.log" 2>&1
}
