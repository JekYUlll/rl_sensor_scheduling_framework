...
LOG_DIR="$ROOT/logs"
LOG_FILE="$LOG_DIR/behaviorbrg3_pilot_local_watch_20260621.log"
STATUS_FILE="$ROOT/reports/aggregate/behaviorbrg3_pilot_local_watch_20260621_status.md"
REMOTE_DIR="~/_code/microclimate_demo/rl_sensor_scheduling_framework"
SESSION="${SESSION:-behaviorbrg3_pilot_parallel_83_92_20260621}"
...
printf '\nTMUX\n'
tmux ls 2>/dev/null | grep -E 'behaviorbrg3|behaviorbrg2|behaviorbrg|behaviorbd|behaviorreg|autoteacher|router|bo24' || true
printf '\nSTATUS bits: oracle ppo base_eval router_eval replay behavior\n'
for seed in 83 84 86 87 91 92; do
  d=reports/v31_metpair_backbone_context_ortholinear_balancedobjective_behaviorbrg3_seed${seed}_h075ctxolbrg3_20260621
  printf 'seed%s ' "$seed"
  ...
  printf ' '
  if test -f "logs/behaviorbrg3_pilot_seed${seed}_20260621.log"; then
    grep -Eo 'timesteps=[0-9]+' "logs/behaviorbrg3_pilot_seed${seed}_20260621.log" | tail -1 | tr -d '\n' || true
  fi
  printf '\n'
done
printf '\nAGG FILES\n'
find reports/aggregate -maxdepth 2 -type f \( -name 'oldclaim_summary.json' -o -name 'metpair_claim_summary.json' -o -name '*.md' \) \
  -path '*behaviorbrg3*20260621*' -printf '%TY-%Tm-%Td %TH:%M %p\n' 2>/dev/null | sort | tail -20
printf '\nLOG TAILS\n'
for seed in 83 84 86 87 91 92; do
  log=logs/behaviorbrg3_pilot_seed${seed}_20260621.log
  printf '%s | ' "$log"
  test -f "$log" && tail -n 3 "$log" | tr '\n' ' ' || printf missing
  printf '\n'
done
printf '\nERROR SCAN\n'
grep -RInE 'Traceback|ERROR|Exception|RuntimeError|CUDA out of memory|nan' logs/behaviorbrg3_pilot_*_20260621.log logs/behaviorbrg3_collect_*_20260621.log logs/behaviorbrg3_write_report_*_20260621.log 2>/dev/null | tail -40 || true
printf '\nPYTHON PROCESSES\n'
ps -fu zhangzhuyu | grep -E "behaviorbrg3|25_v2_train_custom_ppo|64_v31_eval|70_v31_split|71_v31_behavior|73_v31_collect|74_v31_write" | grep -v grep | wc -l
printf '\nGPU\n'
...
sync_aggregates() {
  rsync -az \
    --include '*behaviorbrg3*20260621*/***' \
    --include 'behaviorbrg3_*_20260621.md' \
    --exclude '*' \
...
write_status() {
  {
    printf '# BRG-3 Pilot Watch Status\n\n'
    printf '- Local time: `%s`\n' "$(date -Is)"
