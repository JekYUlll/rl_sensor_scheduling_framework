...
# V338 creates matched control assets for a predeclared scene recalibration.
# It changes event coverage and subtype prevalence only; physical channels,
# effective costs, budgets, dwell rules, and online information are unchanged.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v338_recalibrated_scene_control"
    export LOG_PREFIX="v338_recalibrated_scene_control"
    export TOTAL_TIMESTEPS=1 POLICY_SEED="$((7300 + slot))"
  ...
  ) >"logs/v338_recalibrated_scene_control_seed${seed}.log" 2>&1
}
