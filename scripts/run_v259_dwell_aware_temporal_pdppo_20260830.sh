...
# V259 measures the dwell-aware action-feasibility correction with the same
# structured temporal encoder and physical scene as V258.
# A GRU encodes the deployable observation/mask history while the original
# forecast-loss reward and masked PPO action distribution remain unchanged.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v259_dwell_aware_temporal_pdppo_sixch_dev"
  ...
  ) >"logs/v259_dwell_aware_temporal_pdppo_sixch_seed${seed}.log" 2>&1
}
