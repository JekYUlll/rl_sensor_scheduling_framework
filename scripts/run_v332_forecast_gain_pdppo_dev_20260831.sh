...
# V332 tests one-step counterfactual forecast-gain shaping.  It matches the
# corrected six-channel ordinary PPO protocol and changes only the reward
# proxy from absolute forecast loss to the current-state carried-observation
# counterfactual gain.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6841 6842)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7201 7202})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v332_forecast_gain_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v332_forecast_gain_pdppo}"
  ...
  ) >"logs/v332_forecast_gain_pdppo_seed${seed}.log" 2>&1
}
