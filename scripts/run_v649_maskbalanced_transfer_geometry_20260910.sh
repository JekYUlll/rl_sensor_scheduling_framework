#!/usr/bin/env bash
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
PYTHON_BIN="${PYTHON_BIN:-/home/zhangzhuyu/.conda/envs/darts/bin/python}"
RUN_ROOT="${V649_ASSET_ROOT:-reports/v648_physical_only_assets_maskbalanced_20260910}"
OUT_ROOT="${V649_GEOMETRY_ROOT:-reports/v649_maskbalanced_transfer_geometry_20260910}"
mkdir -p "$OUT_ROOT"
declare -A TRAIN_STARTS=(
  [7401]="1000 1211 7717"
  [7402]="1000 1087 7958"
  [7403]="1000 1198 47626"
  [7404]="1000 1203 7693"
)
declare -A TEST_STARTS=(
  [7401]="82600"
  [7402]="82600"
  [7403]="82600"
  [7404]="82600"
)
for seed in 7401 7402 7403 7404; do
  out="$OUT_ROOT/seed${seed}"
  mkdir -p "$out"
  args=()
  for start in ${TRAIN_STARTS[$seed]}; do args+=(--start-index "$start"); done
  "$PYTHON_BIN" scripts/109_v32_audit_subset_forecast_geometry.py \
    --run-dir "$RUN_ROOT/seed${seed}" --out-dir "$out/train" \
    --steps 256 --max-rollouts 4 "${args[@]}" \
    --steady-budget 10.0 --startup-budget 10.0 --dynamic-resource-budget 55.0 \
    --oracle-loss-clip 1000000000 --torch-threads 1 > "$out/train.log" 2>&1
  "$PYTHON_BIN" scripts/109_v32_audit_subset_forecast_geometry.py \
    --run-dir "$RUN_ROOT/seed${seed}" --out-dir "$out/test" \
    --steps 256 --max-rollouts 1 --start-index "${TEST_STARTS[$seed]}" \
    --steady-budget 10.0 --startup-budget 10.0 --dynamic-resource-budget 55.0 \
    --oracle-loss-clip 1000000000 --torch-threads 1 > "$out/test.log" 2>&1
done

"$PYTHON_BIN" scripts/184_audit_state_winner_transfer.py \
  --run-root "$OUT_ROOT" --assets-root "$RUN_ROOT" --output "$OUT_ROOT/summary"
