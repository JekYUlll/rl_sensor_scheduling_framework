...
# V320 uses only executable decision rows for hard forecast-value
# pretraining, then performs ordinary decision-only PPO updates. This removes
# labels for forced dwell rows without adding any baseline-dependent signal.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7001 7002})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v320_corrected_scene_decision_only_hardpretrain_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v320_corrected_scene_decision_only_hardpretrain}"
    ...
    export BC_PRETRAIN_TARGET_MODE=hard_forecast_value BC_PRETRAIN_DECISION_ONLY=1 BC_SOFT_TEMPERATURE=1.0
  ...
  ) >"logs/v320_corrected_scene_decision_only_hardpretrain_seed${seed}.log" 2>&1
}
