...
# V357 creates frozen cycling-scene control assets for an independent
# confirmation wave. It does not train a learned policy.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v357_confirmation_scene_control"
    export LOG_PREFIX="v357_confirmation_scene_control"
    export TOTAL_TIMESTEPS=1 POLICY_SEED="$((7450 + idx))"
  ...
  ) >"logs/v357_confirmation_scene_control_seed${seed}.log" 2>&1
}

mkdir -p logs
if (( ${#SEEDS[@]} == 0 )); then
  echo "At least one scene seed is required" >&2
  exit 2
fi
pids=()
for i in "${!SEEDS[@]}"; do run_one "$i" "${SEEDS[$i]}" & pids+=("$!"); done
