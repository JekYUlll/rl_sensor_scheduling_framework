...
RUN_TAG=""
PREDICTOR_GPUS="0"
BASE_CFG="configs/base_forecast_first_medium.yaml"
ENV_CFG="configs/env/windblown_case_fulltargets.yaml"
SENSOR_CFG="configs/sensors/windblown_sensors_complex.yaml"
ESTIMATOR_CFG="configs/estimator/kalman.yaml"
REWARD_CFG="configs/reward/lstm_aux_forecast_first_medium.yaml"
PREDICTOR_MODELS="tcn"
...
if [[ -z "${RUN_TAG}" ]]; then
  RUN_TAG="forecast_first_medium_$(date +%Y%m%d_%H%M%S)"
fi
...
echo "PREDICTOR_MODELS=${PREDICTOR_MODELS}"
...
print(int(cfg.get("data", {}).get("truth_steps", 262144)))
PY
)"
...
declare -A SCHED_CFG=(
  ...
  [dqn]="configs/scheduler/dqn_medium.yaml"
  [cmdp_dqn]="configs/scheduler/cmdp_dqn_medium.yaml"
)
...
python scripts/10_posthoc_task_focus.py \
  ...
  --target-set forecast \
  --env-cfg "${ENV_CFG}" \
  | tee "reports/logs/${RUN_TAG}_10_posthoc_forecast_targets.log"

python scripts/10_posthoc_task_focus.py \
  --run-tag "${RUN_TAG}" \
  --target-set primary \
  --env-cfg "${ENV_CFG}" \
  | tee "reports/logs/${RUN_TAG}_10_posthoc_primary_targets.log"

python scripts/11_plot_rl_training_diagnostics.py \
  --run-tag "${RUN_TAG}" \
  | tee "reports/logs/${RUN_TAG}_11_rl_training_plots.log"

echo "Forecast-first medium experiment complete: ${RUN_TAG}"
