...
# V328 evaluates a single masked actor with both state-candidate interaction
# and online quality/context utility. It retains the V325/V327 protocol and
# uses no comparator action, event label, or bandit-dependent objective.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7047 7048})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v328_joint_action_utility_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v328_joint_action_utility_pdppo}"
  ...
  ) >"logs/v328_joint_action_utility_pdppo_seed${seed}.log" 2>&1
}
