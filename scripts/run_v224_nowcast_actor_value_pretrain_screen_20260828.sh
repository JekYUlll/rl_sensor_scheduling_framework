...
# Bounded development screen after V223. The same frozen forecaster now
# initializes the categorical PPO actor logits directly, rather than an
# isolated auxiliary value head. The online state remains three label-free
# meteorological nowcasts; no heuristic action, event label, or bandit prior
# is introduced.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

export SEEDS_OVERRIDE="${SEEDS_OVERRIDE:-2431 2432 2433}"
export RUN_PREFIX_OVERRIDE="${RUN_PREFIX_OVERRIDE:-v224_nowcast_actor_value_pretrain_dev}"
