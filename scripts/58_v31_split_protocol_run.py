...
def ensure_truth(args: argparse.Namespace, truth_path: Path) -> Path:
    ...
    cmd = [
        ...
        *[str(station) for station in args.stations],
        ...
        "--event-subtype-cycle-steps",
        str(int(args.event_subtype_cycle_steps)),
...
def main() -> None:
    ...
    parser.add_argument(
        "--reward-proxy-mode",
        choices=["forecast", "forecast_decision", "forecast_gain", "forecast_block_gain", "forecast_block_relative_gain", "forecast_block_absolute", "aoi", "uncertainty", "coverage", "instant_error"],
        default="forecast",
    )
    ...
    cmd.extend(["--onpolicy-action-value-coef", str(float(args.onpolicy_action_value_coef))])
    cmd.extend(["--onpolicy-action-value-scale", str(float(args.onpolicy_action_value_scale))])
    cmd.append(
