...
def main() -> None:
    ...
    row = {'model': str(args.model_name), 'scheduler': meta.get('scheduler_name', 'unknown'), 'source_run_id': meta.get('run_id', ''), 'dataset_npz': args.series_npz, 'lookback': int(oracle.lookback), 'horizon': int(oracle.horizon), 'n_features': int(len(input_feature_names)), 'n_targets': int(len(target_feature_names)), 'device': str(getattr(getattr(oracle.predictor, 'device', None), 'type', 'unknown')), 'target_columns': json.dumps(target_feature_names, ensure_ascii=False), 'context_features': json.dumps(list(getattr(oracle, 'context_features', [])), ensure_ascii=False), 'input_filter_cfg': json.dumps(dict(getattr(oracle, 'input_filter_cfg', None) or {}), ensure_ascii=False), 'avg_power': float(meta.get('avg_power', float('nan'))), 'total_power': float(meta.get('total_power', float('nan'))), 'coverage_mean': float(meta.get('coverage_mean', float('nan'))), 'trace_P_mean': float(meta.get('trace_P_mean', float('nan'))), 'uncertainty_mean': float(meta.get('uncertainty_mean', float('nan'))), 'full_open_power': float(meta.get('full_open_power', float('nan'))), **metrics, 'rmse_norm': float(metrics_norm['rmse']), 'mae_norm': float(metrics_norm['mae']), 'mape_norm': float(metrics_norm['mape']), 'smape_norm': float(metrics_norm['smape']), 'pearson_h1_mean_norm': float(metrics_norm['pearson_h1_mean']), 'dtw_h1_mean_norm': float(metrics_norm['dtw_h1_mean'])}
