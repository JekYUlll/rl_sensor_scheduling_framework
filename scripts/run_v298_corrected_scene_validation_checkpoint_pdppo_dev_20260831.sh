...
# V298 tests validation-selected checkpointing on the corrected scene. The
# source run supplies only same-scene truth and validation static candidates;
# policy training remains fresh and uses the ordinary point forecast reward.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(6841 6842)
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v298_corrected_scene_validation_checkpoint_pdppo_dev"
    export LOG_PREFIX="v298_corrected_scene_validation_checkpoint_pdppo"
  ...
  ) >"logs/v298_corrected_scene_validation_checkpoint_pdppo_seed${seed}.log" 2>&1
}
