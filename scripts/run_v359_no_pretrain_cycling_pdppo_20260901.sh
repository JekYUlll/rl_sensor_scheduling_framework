...
# V359 is a matched policy-learning diagnostic on the frozen V357 scenes.
# The sole intervention is removal of hard forecast-value pretraining; the
# masked PPO objective, online context, temporal encoder, reward, geometry,
# and macro checkpoint selection remain unchanged.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(${SCENE_SEEDS_OVERRIDE:-6901 6902 6903 6904})
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7461 7462 7463 7464})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v359_no_pretrain_cycling_pdppo"
    export LOG_PREFIX="v359_no_pretrain_cycling_pdppo"
  ...
  ) >"logs/v359_no_pretrain_cycling_pdppo_seed${seed}.log" 2>&1
}
