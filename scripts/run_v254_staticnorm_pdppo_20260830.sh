...
# Bounded follow-up to V251: normalize subtype forecast-loss rewards using
# validation-selected static scales, without training-time AWBC/BC/oracle guidance.
# extend PPO training. The six-channel geometry, scene, constraints, reward,
# and online execution remain unchanged.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v254_staticnorm_pdppo_sixch_dev"
    ...
    export FORECAST_VALUE_HEAD=0 REWARD_LOSS_NORMALIZATION=staticnorm_subtype REWARD_PROXY_MODE=forecast
  ...
  ) >"logs/v251_no_teacher_pdppo_sixch_seed${seed}.log" 2>&1
}
