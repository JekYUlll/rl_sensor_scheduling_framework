...
# V264 emits forecast-loss reward only at genuine decision times after V259's
# executable-mask correction. Forced minimum-dwell rows do not repeat the
# same overlapping forecast window as policy reward.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v264_decision_sampled_forecast_pdppo_sixch_dev"
  ...
  ) >"logs/v264_decision_sampled_forecast_pdppo_sixch_seed${seed}.log" 2>&1
}
