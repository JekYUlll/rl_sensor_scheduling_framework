...
BC_PRETRAIN_FORCED_ACTION_WEIGHT="${BC_PRETRAIN_FORCED_ACTION_WEIGHT:-1.0}"
...
FORECAST_VALUE_RANKING_COEF="${FORECAST_VALUE_RANKING_COEF:-0}"
...
POLICY_ALIGNMENT_AUDIT_OUTPUT="${POLICY_ALIGNMENT_AUDIT_OUTPUT:-}"
...
FORECAST_VALUE_SPECIALIST_TARGET_INNOVATIONS="${FORECAST_VALUE_SPECIALIST_TARGET_INNOVATIONS:-0}"
...
EVENT_SUBTYPE_CYCLE_STEPS="${EVENT_SUBTYPE_CYCLE_STEPS:-0}"
...
ONPOLICY_ACTION_VALUE_COEF="${ONPOLICY_ACTION_VALUE_COEF:-0}"
...
CHANNEL_QUALITY_MODE="${CHANNEL_QUALITY_MODE:-independent}"
...
DECISION_BLOCK_ARGS=(--no-decision-block-credit)
...
if [[ "$CHANNEL_QUALITY_ENABLED" == "1" ]]; then
  QUALITY_ARGS=(
    ...
    --channel-quality-mode "$CHANNEL_QUALITY_MODE"
...
if [[ "$FORECAST_VALUE_STATE" == "1" ]]; then
  ...
  if [[ "$FORECAST_VALUE_SPECIALIST_TARGET_INNOVATIONS" == "1" ]]; then
    OPERATING_STATE_ARGS+=(--forecast-value-specialist-target-innovations)
  fi
fi
...
if [[ "$DIRECT_MASK_ACTION_SCORE" == "1" ]]; then
  DIRECT_MASK_ACTION_SCORE_ARGS=(--direct-mask-action-score)
fi
DIRECT_MASK_ACTION_PRIMARY_ARGS=(--no-direct-mask-action-primary)
if [[ "$DIRECT_MASK_ACTION_PRIMARY" == "1" ]]; then
  DIRECT_MASK_ACTION_PRIMARY_ARGS=(--direct-mask-action-primary)
fi
...
for seed in "${SEEDS[@]}"; do
  ...
  if [[ -n "$POLICY_ALIGNMENT_AUDIT_OUTPUT" ]]; then
    control_args+=(--policy-alignment-audit-output "$POLICY_ALIGNMENT_AUDIT_OUTPUT")
  fi
  ...
  "$PY" scripts/58_v31_split_protocol_run.py \
    ...
    --event-subtype-cycle-steps "$EVENT_SUBTYPE_CYCLE_STEPS" \
    ...
    "${BC_DECISION_ARGS[@]}" \
    --bc-pretrain-forced-action-weight "$BC_PRETRAIN_FORCED_ACTION_WEIGHT" \
    ...
    --forecast-value-ranking-coef "$FORECAST_VALUE_RANKING_COEF" \
    ...
    --quality-context-pooling "$QUALITY_CONTEXT_POOLING" \
