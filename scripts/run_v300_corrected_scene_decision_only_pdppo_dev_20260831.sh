...
# V300 fixes the semi-Markov policy-update interface: actor gradients are
# applied only on epochs where a new mask can actually be executed. The critic
# still trains on the complete transition sequence.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(6861 6862)
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v300_corrected_scene_decision_only_pdppo_dev"
    export LOG_PREFIX="v300_corrected_scene_decision_only_pdppo"
  ...
  ) >"logs/v300_corrected_scene_decision_only_pdppo_seed${seed}.log" 2>&1
}
