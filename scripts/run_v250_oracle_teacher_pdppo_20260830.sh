...
# Bounded follow-up to V249: replace the subtype-static AWBC teacher with the
# forecast-greedy teacher. The algorithm, six-channel action geometry, scene,
# constraints, reward, and PPO training budget remain unchanged.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v250_oracle_teacher_pdppo_sixch_dev"
    ...
    export AWBC_TEACHER_MODE=oracle_greedy
  ...
  ) >"logs/v250_oracle_teacher_pdppo_sixch_seed${seed}.log" 2>&1
}
