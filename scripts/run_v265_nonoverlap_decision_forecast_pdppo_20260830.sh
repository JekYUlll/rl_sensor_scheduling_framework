...
# V265 aligns the frozen forecast horizon with the six-step minimum dwell.
# Combined with decision-sampled reward, each decision receives a six-step
# forecast target before the next executable decision, avoiding horizon/dwell
# overlap in the training target.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v265_nonoverlap_decision_forecast_pdppo_sixch_dev"
    ...
    export AWBC_COEF=0 AWBC_DECAY_TIMESTEPS=0 AWBC_EVENT_ONLY=0 AWBC_LABEL_STRIDE=4
    export BC_PRETRAIN_STEPS=0 BC_PRETRAIN_EPOCHS=0 BC_PRETRAIN_LOSS_COEF=0
    export ORACLE_SUBTYPE_TEACHER_REPEAT=0 ORACLE_SUBTYPE_TEACHER_LOOKAHEAD_STEPS=6
    ...
    export REWARD_LOSS_NORMALIZATION=none REWARD_PROXY_MODE=forecast_decision
    export FORECAST_VALUE_HEAD=0 FORECAST_VALUE_HEAD_MODE=mask_structured
    export FORECAST_VALUE_HEAD_SCALE=0.0 FORECAST_VALUE_HEAD_HIDDEN_DIM=64
    export FORECAST_VALUE_HEAD_IGNORE_QUALITY=0
    export FORECAST_VALUE_AUX_COEF=0.0 FORECAST_VALUE_AUX_STRIDE=64
    export FORECAST_VALUE_AUX_LOOKAHEAD_STEPS=0 FORECAST_VALUE_AUX_LOSS=soft_ce
    export FORECAST_VALUE_AUX_TEMPERATURE=1.0 FORECAST_VALUE_RANKING_COEF=0.10
    export TARGET_WEIGHTS="1 1 1 1 1 3 1 1 1" COMMON_RANDOM_NUMBERS=0
    export TEMPORAL_ENCODER=1 TEMPORAL_HIDDEN_DIM=64
    export SEPARATE_ACTOR_CRITIC_GRAD_CLIP=1
    export GAMMA=0.99 GAE_LAMBDA=0.95 DECISION_ONLY_POLICY_UPDATES=0 DECISION_BLOCK_CREDIT=0 DECISION_BLOCK_REWARD_MODE=sum
    bash scripts/run_v32_flexible_subset_pilot_20260822.sh "$seed"
  ) >"logs/v265_nonoverlap_decision_forecast_pdppo_sixch_seed${seed}.log" 2>&1
}
