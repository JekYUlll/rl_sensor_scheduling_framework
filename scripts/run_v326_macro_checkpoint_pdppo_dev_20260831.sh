...
# V326 tests validation checkpoint selection by the static-normalized macro
# endpoint. The policy, reward, action geometry, and training protocol are
# unchanged from V325; no comparator action or test label is exposed.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7043 7044})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v326_macro_checkpoint_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v326_macro_checkpoint_pdppo}"
    ...
    export CHECKPOINT_SELECTION_INTERVAL_UPDATES=5 CHECKPOINT_SELECTION_SCORE=oracle_loss_macro_subtype_event_staticnorm CHECKPOINT_REQUIRE_VALID_BEHAVIOR=1
  ...
  ) >"logs/v326_macro_checkpoint_pdppo_seed${seed}.log" 2>&1
}
