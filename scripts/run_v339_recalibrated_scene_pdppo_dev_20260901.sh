...
# V339 trains the unchanged clean PD-PPO configuration on the V338 scene.
# Control assets are same-scene and validation-only for checkpoint selection.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6871 6872)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7311 7312})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v339_recalibrated_scene_pdppo_dev"
    export LOG_PREFIX="v339_recalibrated_scene_pdppo"
  ...
  ) >"logs/v339_recalibrated_scene_pdppo_seed${seed}.log" 2>&1
}
