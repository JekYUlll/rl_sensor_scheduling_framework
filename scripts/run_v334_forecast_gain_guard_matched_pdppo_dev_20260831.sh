...
# V334 is the valid matched rerun of the V333 execution-semantics test.  It
# reuses same-scene validation assets so checkpoint selection is operational.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7221 7222})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v334_forecast_gain_guard_matched_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v334_forecast_gain_guard_matched_pdppo}"
  ...
  ) >"logs/v334_forecast_gain_guard_matched_pdppo_seed${seed}.log" 2>&1
}
