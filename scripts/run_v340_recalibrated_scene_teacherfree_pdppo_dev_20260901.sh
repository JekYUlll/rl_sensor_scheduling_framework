...
# V340 isolates PPO credit assignment from the hard forecast-value teacher.
# It keeps the V338 scene, evaluator, action geometry, and online observations
# fixed, while removing only the supervised pretraining stage.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6871 6872)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7331 7332})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v340_recalibrated_scene_teacherfree_pdppo_dev"
    export LOG_PREFIX="v340_recalibrated_scene_teacherfree_pdppo"
  ...
  ) >"logs/v340_recalibrated_scene_teacherfree_pdppo_seed${seed}.log" 2>&1
}
