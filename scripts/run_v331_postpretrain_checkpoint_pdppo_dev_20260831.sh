...
# V331 tests whether selecting the pretraining checkpoint (update 0) is
# contributing to unstable transfer. It retains V330's method and protocol,
# but allows validation selection only after five PPO updates.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7101 7102})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v331_postpretrain_checkpoint_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v331_postpretrain_checkpoint_pdppo}"
  ...
  ) >"logs/v331_postpretrain_checkpoint_pdppo_seed${seed}.log" 2>&1
}
