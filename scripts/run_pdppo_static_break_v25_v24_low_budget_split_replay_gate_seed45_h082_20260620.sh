...
SOURCE_DIR="${SOURCE_DIR:-reports/v31_static_break_v25_v24_low_budget_${PROFILE_NAME}_b${BUDGET_TAG}_zero_ppo_source_seed45_h082_20260620}"
REPLAY_DIR="${REPLAY_DIR:-reports/v31_static_break_v25_v24_low_budget_${PROFILE_NAME}_b${BUDGET_TAG}_split_replay_gate_seed45_h082_20260620}"

mkdir -p "$SOURCE_DIR" "$REPLAY_DIR"

# Generate the same split-run oracle and final-test starts with zero PPO
# timesteps. This is the required pre-PPO replay gate if the V25 low-budget TCN
# scan finds a passing structural point.
