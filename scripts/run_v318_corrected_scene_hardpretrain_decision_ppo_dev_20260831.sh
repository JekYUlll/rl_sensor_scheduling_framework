...
# V318 tests the previously untested combination of feasible hard
# forecast-value initialization followed by ordinary decision-only PPO
# updates.  It does not use bandit actions, test labels, or a learned action
# prior.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-6981 6982})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v318_corrected_scene_hardpretrain_decision_ppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v318_corrected_scene_hardpretrain_decision_ppo}"
  ...
  ) >"logs/v318_corrected_scene_hardpretrain_decision_ppo_seed${seed}.log" 2>&1
}
