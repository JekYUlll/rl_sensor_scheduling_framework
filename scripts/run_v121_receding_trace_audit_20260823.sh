...
for index in "${!SEEDS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="$index" "$PYTHON" scripts/99_v32_receding_upper.py \
      ...
      --output-subdir receding_oracle_l8_validation_trace \
      --device cuda
    CUDA_VISIBLE_DEVICES="$index" "$PYTHON" scripts/99_v32_receding_upper.py \
      --run-dir "$run_dir" \
      --partition final_test \
      --output-subdir receding_oracle_l8_final_trace \
      --device cuda
  ) >"logs/v121_trace_seed${seed}_20260823.log" 2>&1 &
  pids+=("$!")
done
...
"$PYTHON" scripts/102_v32_receding_trace_learnability.py \
  --run-dirs "${run_dirs[@]}" \
  --output-dir reports/aggregate/v121_receding_trace_learnability_20260823
echo "V121_EXIT=0"
