...
# V317 tests the existing semi-Markov decision-block credit path.  It keeps
# the corrected scene, forecast reward, feasibility mask, and static-normalized
# evaluation protocol from V302, replacing step-level actor credit with the
# discounted reward accumulated until the next executable decision.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-6971 6972})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v317_corrected_scene_decision_block_credit_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v317_corrected_scene_decision_block_credit_pdppo}"
    ...
    export DECISION_ONLY_POLICY_UPDATES=0 DECISION_BLOCK_CREDIT=1 DECISION_BLOCK_REWARD_MODE=sum
  ...
  ) >"logs/v317_corrected_scene_decision_block_credit_pdppo_seed${seed}.log" 2>&1
}
