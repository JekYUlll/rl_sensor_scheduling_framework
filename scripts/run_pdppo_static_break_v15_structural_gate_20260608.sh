...
OUT_DIR="reports/v31_static_break_v15_structural_gate_20260608"
mkdir -p "$OUT_DIR"
...
python scripts/63_v31_static_break_calibration.py \
  ...
  --out-dir "$OUT_DIR" \
  --profiles particle_flux_v6 micro_flux_v6 micro_particle_v6 flux_micro_v6 \
  --budgets 1.10 1.15 1.20 1.25 \
  --startup-peak-budgets 1.55 1.65 \
  --max-active 4 \
  --coverage-groups \
  --oracle-type linear \
  --truth-steps 60000 \
