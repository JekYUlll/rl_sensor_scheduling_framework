...
# V281 isolates decision-time policy credit from V280.  The ordinary forecast
# reward remains unchanged; only PPO policy weighting is restricted to epochs
# at which a new executable subset can be selected.  No bandit signal, teacher
# action, or test label is introduced.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v281_decision_only_forecast_pdppo_sixch_dev"
    export LOG_PREFIX="v281_decision_only_forecast_pdppo_sixch"
  ...
  ) >"logs/${LOG_PREFIX:-v281_decision_only_forecast_pdppo_sixch}_seed${seed}.log" 2>&1
}
