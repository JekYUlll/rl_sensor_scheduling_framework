...
# Bounded development screen.  The online policy remains a feasibility-masked
# PPO policy.  A frozen forecast evaluator supplies training-time candidate
# action-value targets; no alert/event label or heuristic action enters state.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"

export SEEDS_OVERRIDE="${SEEDS_OVERRIDE:-2421 2422 2423}"
export RUN_PREFIX_OVERRIDE="${RUN_PREFIX_OVERRIDE:-v223_nowcast_value_pretrain_dev}"
...
export FORECAST_VALUE_HEAD=1
export FORECAST_VALUE_HEAD_MODE=factorized
export FORECAST_VALUE_AUX_COEF=0.20
export FORECAST_VALUE_AUX_STRIDE=32
export FORECAST_VALUE_AUX_LOOKAHEAD_STEPS=4
