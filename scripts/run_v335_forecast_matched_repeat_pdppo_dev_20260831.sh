...
# V335 is a matched repeat of the valid V331 protocol.  It isolates the
# V334 change in reward proxy: the original forecast objective is restored,
# while the scene, checkpoint selection, network, and execution protocol stay
# fixed.  Policy seeds are fresh.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7231 7232})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v335_forecast_matched_repeat_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v335_forecast_matched_repeat_pdppo}"
  ...
  ) >"logs/v335_forecast_matched_repeat_pdppo_seed${seed}.log" 2>&1
}
