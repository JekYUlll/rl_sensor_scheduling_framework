...
LOG_DIR="$ROOT/logs"
LOG_FILE="$LOG_DIR/behaviorreg_pilot_local_watch_20260621.log"
STATUS_FILE="$ROOT/reports/aggregate/behaviorreg_pilot_local_watch_20260621_status.md"
INTERVAL_SECONDS="${INTERVAL_SECONDS:-600}"
MAX_HOURS="${MAX_HOURS:-24}"
SESSION="${SESSION:-behaviorreg_pilot_parallel_83_92_20260621}"
...
sync_aggregates() {
  rsync -az --include '*/' \
    --include '*behaviorreg*20260621*/***' \
    --include 'behaviorreg_*_20260621.md' \
    --exclude '*' \
...
date -Is
echo "tmux:"
tmux ls 2>/dev/null | grep -E 'behaviorreg|autoteacher|router|bo24' || true
echo
echo "seed_status: oracle ppo base_eval router_eval replay behavior"
for seed in 83 84 86 87 91 92; do
  d=reports/v31_metpair_backbone_context_ortholinear_balancedobjective_behaviorreg_seed${seed}_h075ctxolboreg_20260621
  printf 'seed%s ' "$seed"
...
echo "latest_logs:"
for seed in 83 84 86 87 91 92; do
  log=logs/behaviorreg_pilot_seed${seed}_20260621.log
  printf '%s | ' "$log"
...
find reports/aggregate -maxdepth 2 -type f \( -name 'oldclaim_summary.json' -o -name 'metpair_claim_summary.json' -o -name '*.md' \) \
  -path '*behaviorreg*20260621*' -printf '%TY-%Tm-%Td %TH:%TM %p\n' 2>/dev/null | sort | tail -20
echo
echo "gpu:"
nvidia-smi --query-gpu=index,utilization.gpu,memory.used --format=csv,noheader 2>/dev/null || true
echo
echo "active_count:"
ps -fu zhangzhuyu | grep -E "behaviorreg|25_v2_train_custom_ppo|64_v31_eval|70_v31_split|71_v31_behavior|73_v31_collect|74_v31_write" | grep -v grep | wc -l
REMOTE
}
...
while [ "$(date +%s)" -lt "$end_epoch" ]; do
  {
    echo "# Behavior-Regularized PPO Pilot Local Watch Status"
    echo
