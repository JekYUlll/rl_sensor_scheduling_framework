...
# V263 assigns semi-Markov credit to genuine decision times after V259's
# executable-mask correction. The forecast-loss reward, action geometry, and
# policy inputs are unchanged; critic learning still uses every rollout row.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v263_terminal_dwell_credit_pdppo_sixch_dev"
    export TOTAL_TIMESTEPS=50000 TRUTH_STEPS=36000 LOOKBACK=20
    ...
    export REWARD_LOSS_NORMALIZATION=none REWARD_PROXY_MODE=forecast
    ...
    export GAMMA=0.99 GAE_LAMBDA=0.95 DECISION_ONLY_POLICY_UPDATES=0 DECISION_BLOCK_CREDIT=1 DECISION_BLOCK_REWARD_MODE=terminal
    bash scripts/run_v32_flexible_subset_pilot_20260822.sh "$seed"
  ) >"logs/v263_terminal_dwell_credit_pdppo_sixch_seed${seed}.log" 2>&1
}
