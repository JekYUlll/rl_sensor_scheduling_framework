...
# V304 tests the existing forecast-value soft action-ranking auxiliary on the
# corrected six-channel scene. It retains the forecast-loss reward, masked PPO
# action space, and validation-only checkpoint selection; no bandit action or
# final-test label is used.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-6901 6902})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v304_corrected_scene_staticnorm_softvalue_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v304_corrected_scene_staticnorm_softvalue_pdppo}"
    ...
    export FORECAST_VALUE_AUX_COEF=0.20 FORECAST_VALUE_AUX_STRIDE=32 FORECAST_VALUE_AUX_LOOKAHEAD_STEPS=6
    export FORECAST_VALUE_AUX_LOSS=soft_ce FORECAST_VALUE_AUX_TEMPERATURE=0.75 FORECAST_VALUE_RANKING_COEF=0
  ...
  ) >"logs/v304_corrected_scene_staticnorm_softvalue_pdppo_seed${seed}.log" 2>&1
}
