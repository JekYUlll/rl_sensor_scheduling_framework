...
# V257 tests a learned candidate-level forecast-cost head.  The head is trained
# from frozen-forecaster costs collected sparsely during policy rollouts and is
# fused with the masked PPO logits.  It uses no bandit action, bandit margin,
# counterfactual label, or test-time event label.
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v257_dense_mask_forecast_head_pdppo_sixch_dev"
    ...
    export FORECAST_VALUE_HEAD=1 FORECAST_VALUE_HEAD_MODE=mask_structured
    export FORECAST_VALUE_HEAD_SCALE=0.35 FORECAST_VALUE_HEAD_HIDDEN_DIM=64
    export FORECAST_VALUE_HEAD_IGNORE_QUALITY=0
    export FORECAST_VALUE_AUX_COEF=0.10 FORECAST_VALUE_AUX_STRIDE=8
  ...
  ) >"logs/v257_dense_mask_forecast_head_pdppo_sixch_seed${seed}.log" 2>&1
}
