...
# V324 replaces V323's fixed forecast-head fusion with a learned state-
# dependent trust gate. The gate is driven by the actor representation and
# trained through the ordinary PPO objective; no comparator action or label is
# exposed to the policy.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
SEEDS=(6811 6812)
POLICY_SEEDS=(${POLICY_SEEDS_OVERRIDE:-7041 7042})
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="${RUN_PREFIX_OVERRIDE:-v324_state_dependent_teacher_trust_pdppo_dev}"
    export LOG_PREFIX="${LOG_PREFIX_OVERRIDE:-v324_state_dependent_teacher_trust_pdppo}"
    ...
    export FORECAST_VALUE_TRUST_GATE=1 FORECAST_VALUE_TRUST_HIDDEN_DIM=64
  ...
  ) >"logs/v324_state_dependent_teacher_trust_pdppo_seed${seed}.log" 2>&1
}
