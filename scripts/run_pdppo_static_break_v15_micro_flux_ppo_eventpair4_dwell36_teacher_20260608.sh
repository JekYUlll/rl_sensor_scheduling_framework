...
OUT_DIR=reports/v31_static_break_v15_micro_flux_ppo_eventpair4_dwell36_teacher_20260608

"$PY" scripts/59_v31_split_protocol_grid.py \
  --out-dir "$OUT_DIR" \
  --sensor-cfg configs/sensors/windblown_sensors_physical_event_v15_event_complementarity.yaml \
  --budgets 1.15 \
  --seeds 41 \
  --workers 1 \
  --gpu-ids 5 \
  ...
  --soc-aux-horizon 32 \
  --soc-aux-coef 0.03 \
  --lambda-warmup-abort 0.20 \
  --lambda-switch 0.002 \
  --energy-account \
  --energy-capacity 180 \
  --initial-energy 180 \
  --harvest-per-step 0.65 \
  --reserve-energy 20 \
  ...
  --awbc-teacher-calm-sensors met_station_core radiometer_basic surface_temp_ir snow_particle_counter \
  --awbc-teacher-event-sensors met_station_core radiometer_basic laser_disdrometer \
  ...
  --eval-rollouts 4 \
  --ent-coef 0.001 \
  --target-weights 0.03 0.03 0.10 0.01 0.01 0.0 18.0 12.0 12.0 \
...
"$PY" scripts/65_v31_collect_operational_pdppo.py \
  ...
  --seeds 41 \
  --out-name v15_micro_flux_b1p15_seed41_eventpair4_dwell36_teacher_summary.csv
