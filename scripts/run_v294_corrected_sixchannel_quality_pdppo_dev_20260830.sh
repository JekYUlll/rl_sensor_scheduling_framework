...
# V294 regenerates the six-channel scene after correcting the physical
# channel-quality map. It keeps V279's ordinary PPO protocol and tests only
# whether the corrected scene changes the static-shortcut behaviour.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
if [[ "$#" -gt 0 ]]; then SEEDS=("$@"); else SEEDS=(6811 6812); fi

run_one() {
  local seed="$1" gpu="$2"
  (
    export CUDA_VISIBLE_DEVICES="$gpu"
    export RUN_PREFIX="v294_corrected_sixchannel_quality_pdppo_dev"
    export LOG_PREFIX="v294_corrected_sixchannel_quality_pdppo"
  ...
  ) >"logs/v294_corrected_sixchannel_quality_pdppo_seed${seed}.log" 2>&1
}
