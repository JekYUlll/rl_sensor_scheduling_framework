...
# Full PD-PPO confirmation on the corrected balanced quality scene with the
# physical six-channel arbitrary-subset geometry.  The
# training-only guide and auxiliary losses are part of the complete method;
# final execution receives only online context and carried observations.
...
if [[ "$#" -gt 0 ]]; then
  ...
  SEEDS=(3801 3802 3803 3804 3805)
fi
...
run_one() {
  ...
  (
    ...
    export RUN_PREFIX="v248_full_pdppo_sixch_dev"
  ...
  ) >"logs/v248_full_pdppo_sixch_seed${seed}.log" 2>&1
}
