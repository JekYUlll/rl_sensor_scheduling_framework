...
# V302 tests validation-frozen subtype reward normalization after the V300
# decision-only update-interface correction.  No baseline-dependent reward,
# action prior, or test-time information is introduced.
...
run_one() {
  ...
  ) >"logs/v302_corrected_scene_decision_only_staticnorm_pdppo_seed${seed}.log" 2>&1
}
