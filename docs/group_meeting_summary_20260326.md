...
把当前实验写成更严格的建模语言，可以概括为：

- 场景：南极 AWS 多传感器观测 + 供电受限，我们要在每个时刻决定“开哪些传感器”；
- 建模：这是一个序贯决策问题，可视为 MDP；加入功率与能量约束后，更接近 CMDP；
- 状态 `state`：仅使用当前可获得信息，不使用未来真值；当前实现包含 `x_hat`、`diag(P)`、`trace(P)`、freshness、coverage、budget ratio、上一动作掩码、事件指示；
- 动作 `action`：当前时刻的传感器开关子集；在 windblown 中由策略打分后，经 `OnlineSubsetProjector` 投影成可执行子集；
- 可行域约束：瞬时功率上限、启动峰值功率上限、安全裕度；`cmdp_dqn` 额外处理平均功率/总能量约束；
- 转移 `transition`：执行动作后得到部分观测，时间推进，传感器状态与估计状态更新（truth replay 是确定性的）；
- 奖励 `reward`：目标不是“采得越多越好”，而是“在受限功耗下保留任务相关信息”；当前默认以主任务状态误差 + 切换代价 + 覆盖惩罚为主，功率更多通过约束层体现；
- 求解：规则基线（`full_open/random/periodic/round_robin/info_priority`）+ 学习基线（`dqn/cmdp_dqn/ppo`）；
- 训练与评估：先在回放环境学习调度，再生成调度后数据集训练下游模型（TCN/LSTM/Transformer/Informer 等），最后做 aggregate 对比；
- 结论口径：比较同一功率预算下的误差、节电率、约束违背率及相对 `full_open` 的差距；RL 的价值是资源受限下逼近上限，而不是超过全观测上限。
