...
class OnlineSubsetProjector:
    ...
    def __init__(
        ...
        exact_search_max_sensors: int = 12,
    ) -> None:
    ...
    def feasible_subsets(self, prev_selected: Iterable[str] | None = None) -> list[tuple[str, ...]]:
        return [subset for subset in self._subset_cache if self.is_subset_feasible(subset, prev_selected)]

    def size(self) -> int:
        # Kept for interface compatibility only; windblown RL should not use this
        return len(self._subset_cache)

    def sample_random_subset(self, prev_selected: Iterable[str] | None = None, rng: np.random.Generator | None = None) -> list[str]:
        feasible = self.feasible_subsets(prev_selected)
        if not feasible:
            return []
    ...
    def select_from_scores(self, score_map: dict[str, float], prev_selected: Iterable[str] | None = None) -> list[str]:
        ...
        if len(self.sensor_ids) <= self.exact_search_max_sensors:
            ...
            for subset in self.feasible_subsets(prev_selected):
                score = self._subset_score(subset, ordered_scores)
                if score > best.score + 1e-12 or (
                    abs(score - best.score) <= 1e-12 and len(subset) > len(best.sensors)
                ):
                    best = SensorSubset(tuple(subset), score)
            return list(best.sensors)
        # Greedy fallback for larger sensor sets.
