...
class FrozenForecastRewardOracle:
    ...
    time_delta_cfg: dict[str, object] | None = None
    base_freq_s: int = 1
    ...
    def score(
        ...
        input_prepared, input_names, _, _, _ = prepare_input_and_targets(
            ...
            time_delta_cfg=self.time_delta_cfg,
        )
        future_time = None
...
def _load_single_reward_oracle(artifact_path: str | Path) -> FrozenForecastRewardOracle:
    ...
    time_delta_cfg = payload.get('time_delta_cfg')
    if time_delta_cfg is None:
        time_delta_cfg = dict(payload.get('predictor_cfg', {}).get('time_delta', {}) or {}) or None
    if time_delta_cfg is None and any(str(name).startswith('delta_') for name in payload['input_columns']):
        # Older artifacts used unbounded raw delta_* features.  Keep those
        # artifacts reproducible instead of silently feeding bounded staleness.
        time_delta_cfg = {'encoding': 'raw', 'prefix': 'delta'}
    return FrozenForecastRewardOracle(predictor=predictor, lookback=int(payload['lookback']), horizon=horizon, base_feature_names=[str(v) for v in payload.get('base_feature_names', payload['input_columns'])], input_columns=[str(v) for v in payload['input_columns']], target_columns=[str(v) for v in payload['target_columns']], x_mean=np.asarray(payload['x_mean'], dtype=np.float32), x_std=np.asarray(payload['x_std'], dtype=np.float32), y_mean=np.asarray(payload['y_mean'], dtype=np.float32), y_std=np.asarray(payload['y_std'], dtype=np.float32), loss_name=str(payload.get('loss_name', 'huber')), loss_delta=float(payload.get('loss_delta', 1.0)), use_observed_mask=bool(payload.get('use_observed_mask', False)), use_time_delta=bool(payload.get('use_time_delta', False)), horizon_weights=weights, context_features=[str(v) for v in payload.get('context_features', [])], input_filter_cfg=None if input_filter_cfg is None else dict(input_filter_cfg), time_delta_cfg=None if time_delta_cfg is None else dict(time_delta_cfg), base_freq_s=int(payload.get('base_freq_s', 1)), score_scale=float(payload.get('score_scale', 1.0)), score_clip=float(payload.get('score_clip', 50.0)))
