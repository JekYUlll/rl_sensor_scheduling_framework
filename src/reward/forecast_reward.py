...
class FrozenForecastRewardOracle:
    ...
    def score(self, history_window: np.ndarray, future_truth: np.ndarray) -> float:
        ...
        if not self.ready(history.shape[0], future.shape[0]):
            return 0.0
        x = history[-self.lookback :].reshape(1, self.lookback, len(self.input_columns))
        y = future[: self.horizon].reshape(1, self.horizon, self.target_dim)
