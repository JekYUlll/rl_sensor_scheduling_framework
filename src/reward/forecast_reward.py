...
class FrozenForecastRewardOracle:
    ...
    def score(self, history_window: np.ndarray, future_truth: np.ndarray, observed_mask_window: np.ndarray | None=None, time_index_window: np.ndarray | None=None) -> float:
        ...
        input_prepared, input_names, _, _, _ = prepare_input_and_targets(input_series=history_window, target_series=history_window, feature_names=list(self.base_feature_names), observed_mask=mask_window, use_observed_mask=self.use_observed_mask, use_time_delta=self.use_time_delta, target_columns=[], time_index=time_window, base_freq_s=int(self.base_freq_s))
        future_time = None
...
class FrozenForecastRewardEnsemble:
    ...
    def score(self, history_window: np.ndarray, future_truth: np.ndarray, observed_mask_window: np.ndarray | None=None, time_index_window: np.ndarray | None=None) -> float:
        ...
        for oracle, weight in zip(self.oracles, self.weights, strict=True):
            total += float(weight) * float(oracle.score(history_window, future_truth, observed_mask_window, time_index_window))
        return float(total)
...
def _load_single_reward_oracle(artifact_path: str | Path) -> FrozenForecastRewardOracle:
    ...
    return FrozenForecastRewardOracle(predictor=predictor, lookback=int(payload['lookback']), horizon=horizon, base_feature_names=[str(v) for v in payload.get('base_feature_names', payload['input_columns'])], input_columns=[str(v) for v in payload['input_columns']], target_columns=[str(v) for v in payload['target_columns']], x_mean=np.asarray(payload['x_mean'], dtype=np.float32), x_std=np.asarray(payload['x_std'], dtype=np.float32), y_mean=np.asarray(payload['y_mean'], dtype=np.float32), y_std=np.asarray(payload['y_std'], dtype=np.float32), loss_name=str(payload.get('loss_name', 'huber')), loss_delta=float(payload.get('loss_delta', 1.0)), use_observed_mask=bool(payload.get('use_observed_mask', False)), use_time_delta=bool(payload.get('use_time_delta', False)), horizon_weights=weights, base_freq_s=int(payload.get('base_freq_s', 1)), score_scale=float(payload.get('score_scale', 1.0)), score_clip=float(payload.get('score_clip', 50.0)))
...
def load_reward_oracle(artifact_path: str | Path) -> FrozenForecastRewardOracle | FrozenForecastRewardEnsemble:
    ...
    if path.suffix.lower() in {'.yaml', '.yml'}:
        ...
        weights: list[float] = []
        for item in entries:
            item_path = Path(str(item['artifact_path']))
...
def train_reward_oracle_from_series(input_series: np.ndarray, target_series: np.ndarray, input_columns: list[str], target_columns: list[str], reward_cfg: dict, artifact_path: str | Path, observed_mask: np.ndarray | None=None, time_index: np.ndarray | None=None) -> dict:
    ...
    artifact = save_reward_oracle_artifact(predictor=predictor, predictor_cfg=predictor_cfg, stats=stats, lookback=lookback, horizon=horizon, base_feature_names=[str(name) for name in input_columns], input_columns=input_columns_prepared, target_columns=target_columns_prepared, loss_name=str(reward_cfg.get('loss', 'huber')), loss_delta=float(reward_cfg.get('loss_delta', 1.0)), score_scale=score_scale, score_clip=score_clip, metrics=metrics, artifact_path=artifact_path, base_freq_s=base_freq_s)
