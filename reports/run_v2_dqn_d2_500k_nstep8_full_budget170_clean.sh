...
BASE_OUT=reports/v2_forecast_eval_grid_dqn_d2_500k_nstep8_full_clean170
mkdir -p "$BASE_OUT"

run_seed() {
  local seed="$1"
  local gpu="$2"
  echo "[D2 clean budget1.70 seed ${seed}] start on GPU ${gpu}"
  ...
  echo "[D2 clean budget1.70 seed ${seed}] done"
}
...
echo "D2 clean budget=1.70 branch complete."
