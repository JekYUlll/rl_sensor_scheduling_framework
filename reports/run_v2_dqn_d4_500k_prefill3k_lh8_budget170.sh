...
BASE_OUT=reports/v2_forecast_eval_grid_dqn_d4_500k_prefill3k_lh8_full_b170
mkdir -p "$BASE_OUT"

run_seed() {
  local seed="$1"
  local gpu="$2"
  echo "[D4 budget1.70 seed ${seed}] start on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="$gpu" python scripts/26_v2_grid_experiment.py \
    ...
    --oracle-prefill-steps 3000 \
    --oracle-prefill-lookahead-steps 8 \
    --eval-steps 1024 \
    --eval-rollouts 6 \
    ...
    > "$BASE_OUT/budget1p70_seed${seed}.log" 2>&1
  echo "[D4 budget1.70 seed ${seed}] done"
}
...
echo "D4 budget=1.70 branch complete."
